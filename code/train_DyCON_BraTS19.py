...
if __name__ == "__main__":
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            #New computation of uncertainty map (RAIS)
            # student probs
            p_s = F.softmax(stud_logits, dim=1)  # (B, C, H, W, D)
            # gambling softmax over classes
            p_gs = dycon_losses.gambling_softmax(p_s)  
            # entropy per voxel
            entropy = -torch.sum(p_gs * torch.log(p_gs + 1e-6), dim=1, keepdim=True)  # (B,1,H,W,D)

            # downsample entropy map to feature resolution (same factor as you use for mask_con)
            entropy_ds = F.interpolate(
                entropy, 
                scale_factor=1/(args.feature_scaler * 4),
                mode='trilinear',
                align_corners=False
            ).squeeze(1)  # (B, n1, n2, n3)

            gambling_uncertainty = entropy_ds.view(B, -1)  # (B, N)
            ...
            f_loss = fecl_criterion(
                feat=stud_embedding[labeled_bs:],
                mask=mask_con[labeled_bs:],
                teacher_feat=teacher_feat[labeled_bs:] if teacher_feat is not None else None,
                gambling_uncertainty=gambling_uncertainty[labeled_bs:],
                epoch=epoch_num
            )
